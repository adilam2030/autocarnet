...
const buildSalesReportHtml=(car,ops=[])=>{
  ...
  const brand=(car.marque||'MARQUE').toUpperCase();
  const model=car.modele||'';
...
.page{padding:22px 26px;box-sizing:border-box;min-height:1080px;position:relative}
...
.brand-main{font-size:30px;font-weight:900;color:#08244a}
.brand-model{font-size:14px;font-weight:800;color:#b91c1c;margin-top:3px}
.title{flex:1;padding-left:24px}
.title h1{font-size:28px;color:#08244a;margin:0}
.title h2{font-size:15px;color:#334155;margin:6px 0 0}
.title p{font-size:12px;color:#64748b;line-height:1.5;margin:8px 0 0}
.date-box{
  width:145px;
  background:#f8fafc;
...
.vehicle-card,.health-card,.box,.history-card,.note{
  border:1px solid #d9e1ea;
  border-radius:16px;
  background:#fff;
}
.vehicle-card{padding:20px}
.vehicle-name{font-size:36px;font-weight:900;color:#08244a}
.vehicle-year{font-size:20px;color:#475569;margin-top:2px}
...
.meta-value{font-size:14px;font-weight:800;color:#08244a;margin-top:5px}

.health-card{
  padding:20px;
  text-align:center;
}
.health-title{font-size:13px;font-weight:900;color:#08244a}
.score{
...
.health-label{font-size:20px;font-weight:900;color:#16a34a}
.health-small{font-size:12px;color:#64748b;margin-top:8px;line-height:1.4}
...
.kpi-value{font-size:14px;font-weight:900;color:#08244a;margin-top:5px}
.kpi-label{font-size:10.5px;color:#475569;line-height:1.35;margin-top:3px}
...
.history-title h2{font-size:24px;color:#08244a;margin:0}
.history-title p{font-size:12px;color:#64748b;margin:4px 0 0}
.history-grid{display:grid;grid-template-columns:1fr 1fr;gap:16px}
.history-card{padding:13px;page-break-inside:avoid}
.advice{
  margin-top:18px;
  border:1px solid #d9e1ea;
  border-radius:16px;
...
.advice h3{color:#08244a;margin:0 0 8px}
.advice p{font-size:12px;color:#334155;line-height:1.55;margin:0}
</style>
</head>
...
<div class="page">
  <div class="header">
    ...
    <div class="title">
      ...
      <h2>Rapport professionnel de suivi et d’entretien</h2>
      <p>Présentation claire du suivi du véhicule destinée à valoriser l’entretien et la transparence lors de la vente.</p>
    </div>
    <div class="date-box">
  ...
  <div class="top-grid">
    <div class="vehicle-card">
      ...
      <div class="vehicle-year">${car.annee||''}</div>
      <div class="meta-grid">
        <div class="meta"><div class="meta-label">Immatriculation</div><div class="meta-value">${fmtImmat(car.immat)}</div></div>
        <div class="meta"><div class="meta-label">Kilométrage</div><div class="meta-value">${fmtKm(car.km)}</div></div>
        <div class="meta"><div class="meta-label">Carburant</div><div class="meta-value">${car.carburant||'-'}</div></div>
        <div class="meta"><div class="meta-label">Mise en circulation</div><div class="meta-value">${fmtDate(car.dateMEC)}</div></div>
      </div>
    </div>

    <div class="health-card">
      <div class="health-title">ÉTAT DE SANTÉ GLOBAL</div>
      <div class="score">${score}</div>
      <div class="health-label">${label}</div>
      <div class="health-small">Basé sur l’historique d’entretien et les échéances renseignées</div>
    </div>
  </div>

  <div class="kpis">
    <div class="kpi"><div class="kpi-icon">🔧</div><div class="kpi-value">${allOps.length}</div><div class="kpi-label">Interventions</div></div>
    <div class="kpi"><div class="kpi-icon">📋</div><div class="kpi-value">${allOps.length}</div><div class="kpi-label">Historique documenté</div></div>
    <div class="kpi"><div class="kpi-icon">🗓️</div><div class="kpi-value">${nextRevision}</div><div class="kpi-label">Prochaine révision</div></div>
    <div class="kpi"><div class="kpi-icon">🛡️</div><div class="kpi-value">${fmtDate(car.assurance?.echeance)}</div><div class="kpi-label">Assurance</div></div>
    <div class="kpi"><div class="kpi-icon">🏷️</div><div class="kpi-value">${car.vignette?.payee?'Payée':'À vérifier'}</div><div class="kpi-label">Vignette</div></div>
  </div>

  <div class="middle-grid">
    <div class="box">
      <h3>POINTS CLÉS POUR L’ACHETEUR</h3>
      <ul>${positives.slice(0,7).map(x=>`<li>${x}</li>`).join('')}</ul>
    </div>

    <div class="box">
      <h3>FICHE D’IDENTITÉ DU VÉHICULE</h3>
      <table>
        <tr><td>Marque / Modèle</td><td><b>${car.marque} ${car.modele}</b></td></tr>
        <tr><td>Année</td><td><b>${car.annee||'-'}</b></td></tr>
        <tr><td>Immatriculation</td><td><b>${fmtImmat(car.immat)}</b></td></tr>
        <tr><td>Carburant</td><td><b>${car.carburant||'-'}</b></td></tr>
        <tr><td>Kilométrage actuel</td><td><b>${fmtKm(car.km)}</b></td></tr>
        <tr><td>Mise en circulation</td><td><b>${fmtDate(car.dateMEC)}</b></td></tr>
        <tr><td>Carnet d’entretien</td><td><b>Oui - AutoCarnet</b></td></tr>
      </table>
    </div>

    <div class="box">
      <h3>ÉCHÉANCES & SUIVI</h3>
      <table>
        <tr><td>Prochaine révision</td><td><b>${nextRevision}</b></td><td class="status">${revStatus(car)==='ok'?'OK':revStatus(car)==='warning'?'À suivre':'Urgent'}</td></tr>
        <tr><td>Assurance</td><td><b>${fmtDate(car.assurance?.echeance)}</b></td><td class="status">${assStatus(car)==='ok'?'OK':assStatus(car)==='warning'?'À suivre':'Urgent'}</td></tr>
        <tr><td>Contrôle technique</td><td><b>${carAge(car.dateMEC)<5?'Non applicable':fmtDate(car.vt?.echeance)}</b></td><td class="status">${carAge(car.dateMEC)<5?'N/A':vtStatus(car)==='ok'?'OK':'À suivre'}</td></tr>
        <tr><td>Vignette</td><td><b>${car.vignette?.payee?'Payée':'À vérifier'}</b></td><td class="status">${car.vignette?.payee?'OK':'À suivre'}</td></tr>
      </table>
    </div>
  </div>

  <div class="note">Le suivi régulier du véhicule renforce la lisibilité de son historique et la confiance de l’acheteur.</div>
...
<div class="page">
  <div class="history-title">
    ...
    <div>
      ...
      <p>Détail des opérations d’entretien et d’interventions enregistrées</p>
    </div>
  </div>

  <div class="history-grid">
    ${historyHtml || '<div class="history-card"><h3>Aucun historique renseigné</h3></div>'}
  </div>

  <div class="advice">
    <h3>NOTRE AVIS</h3>
    <p>Ce véhicule bénéficie d’un suivi ${score>=85?'rigoureux et complet':score>=70?'globalement rassurant':score>=55?'correct mais à compléter':'à contrôler'}. Les informations saisies permettent de présenter un historique clair et lisible à l’acheteur.</p>
  </div>
