...
import {
  ...
  FlatList, Modal, ActivityIndicator, Share, Platform,
} from 'react-native';
...
const SAFE_BOTTOM=Platform.OS==='android'?12:0;
...
const handleSave=async()=>{
  ...
  return(
    <SafeAreaView style={{flex:1,backgroundColor:'#fff',paddingTop:SAFE_TOP,paddingBottom:SAFE_BOTTOM}}>
      ...
      <ScrollView contentContainerStyle={{padding:20}} keyboardShouldPersistTaps="handled">
        <Card style={{alignItems:'center',padding:14,marginBottom:16}}>
          {form.photoUri?
            <View style={{width:'100%',height:170,borderRadius:14,overflow:'hidden',backgroundColor:'#f3f4f6'}}>
              <Image
...
const healthLabel=(score)=>{
  if(score>=85)return 'Très bon suivi';
  if(score>=70)return 'Bon état général';
  if(score>=55)return 'Suivi correct avec points à vérifier';
  return 'Véhicule à contrôler avant vente';
};

const ReportTitleCard=({icon,title,subtitle,onPress})=>(
  <TouchableOpacity onPress={onPress} activeOpacity={0.85} style={{backgroundColor:'#fff',borderRadius:18,padding:18,marginBottom:12,shadowColor:'#000',shadowOffset:{width:0,height:2},shadowOpacity:0.07,shadowRadius:8,elevation:3}}>
    <View style={{flexDirection:'row',alignItems:'center',gap:14}}>
      <View style={{width:48,height:48,borderRadius:14,backgroundColor:C.primaryLight,alignItems:'center',justifyContent:'center'}}>
        <Text style={{fontSize:24}}>{icon}</Text>
      </View>
      <View style={{flex:1}}>
        <Text style={{fontSize:16,fontWeight:'900',color:C.text}}>{title}</Text>
        <Text style={{fontSize:12,color:C.textLight,marginTop:3,lineHeight:17}}>{subtitle}</Text>
      </View>
...
const buildSalesReportHtml=(car,ops=[])=>{
  const allOps=[...ops].sort((a,b)=>new Date(b.date)-new Date(a.date));
  const normalize=t=>String(t||'').toLowerCase();
  const score=healthScore(car,ops);
  const label=healthLabel(score);
  const todayFr=fmtDate(today());

  const brand=(car.marque||'AUTO').toUpperCase();
  const initials=brand;

  const cat={
    revision:allOps.filter(o=>normalize(o.type).includes('vidange')||normalize(o.type).includes('révision')||normalize(o.type).includes('revision')),
    pneus:allOps.filter(o=>normalize(o.type).includes('pneu')),
    freins:allOps.filter(o=>normalize(o.type).includes('frein')),
    batterie:allOps.filter(o=>normalize(o.type).includes('batterie')),
    clim:allOps.filter(o=>normalize(o.type).includes('clim')),
    distribution:allOps.filter(o=>normalize(o.type).includes('courroie')||normalize(o.type).includes('distribution')),
    boite:allOps.filter(o=>normalize(o.type).includes('boîte')||normalize(o.type).includes('boite')),
    liquides:allOps.filter(o=>normalize(o.type).includes('liquide')||normalize(o.type).includes('refroidissement')),
    admin:allOps.filter(o=>normalize(o.type).includes('assurance')||normalize(o.type).includes('vignette')||normalize(o.type).includes('visite technique')),
  };
  const used=new Set(Object.values(cat).flat().map(o=>o.id));
  cat.autres=allOps.filter(o=>!used.has(o.id));

  const total=allOps.reduce((x,o)=>x+(Number(o.montant)||0),0);
  const nextRevision=`${fmtKm(nextRevKm(car))}`;
  const statusColor=score>=85?'#0f9f55':score>=70?'#2563eb':score>=55?'#d97706':'#dc2626';

  const vehicleRows=[
    ['Marque / Modèle',`${car.marque||'-'} ${car.modele||''}`],
    ['Année',car.annee||'-'],
    ['Immatriculation',fmtImmat(car.immat)],
    ['Carburant',car.carburant||'-'],
    ['Kilométrage actuel',fmtKm(car.km)],
    ['Mise en circulation',fmtDate(car.dateMEC)],
    ['Couleur extérieure',car.colorName||'-'],
    ['Carnet d’entretien','Oui - AutoCarnet'],
  ];

  const deadlineRows=[
    ['Prochaine révision',nextRevision,revStatus(car)],
    ['Assurance',fmtDate(car.assurance?.echeance),assStatus(car)],
    ['Contrôle technique',carAge(car.dateMEC)<5?'Non applicable':fmtDate(car.vt?.echeance),vtStatus(car)],
    ['Vignette',car.vignette?.payee?'Payée':'À vérifier',car.vignette?.payee?'ok':'warning'],
  ];

  const positives=[];
  if(revStatus(car)==='ok')positives.push('Entretien régulier et prochaine révision maîtrisée');
  if(cat.revision.length>0)positives.push('Historique des vidanges / révisions documenté');
  if(cat.pneus.length>0)positives.push('Interventions pneus enregistrées');
  if(cat.freins.length>0)positives.push('Suivi du freinage renseigné');
  if(assStatus(car)==='ok')positives.push('Assurance à jour');
  if(vtStatus(car)==='ok'||carAge(car.dateMEC)<5)positives.push('Contrôle technique conforme selon les informations saisies');
  if(car.vignette?.payee)positives.push('Vignette renseignée comme payée');
  if(positives.length<4)positives.push('Informations centralisées et présentables à l’acheteur');

  const tableCat=(title,list)=>{
    if(!list.length)return '';
    const t=list.reduce((x,o)=>x+(Number(o.montant)||0),0);
    return `
      <div class="hist-box">
        <div class="hist-title">${title}</div>
        <table>
          <thead><tr><th>Date</th><th>Km</th><th>Opération</th><th>Garage</th></tr></thead>
          <tbody>
            ${list.map(o=>`
              <tr>
            ...
            <tr class="total"><td colspan="4">${list.length} intervention(s) enregistrée(s)</td></tr>
          </tbody>
        </table>
      </div>`;
  };
...
.page{min-height:1085px;padding:18px 20px;box-sizing:border-box;position:relative}
.header{display:flex;align-items:center;border-bottom:2px solid #e5e7eb;padding-bottom:14px}
.logo{width:118px;height:82px;border-radius:24px;background:linear-gradient(135deg,#111827,#64748b);color:#fff;display:flex;align-items:center;justify-content:center;font-weight:900;font-size:22px;margin-right:22px;letter-spacing:1px;text-align:center}
.title{flex:1}
.title h1{font-size:28px;margin:0;color:#08244a;letter-spacing:.3px}
.title .sub{font-size:15px;margin-top:4px;color:#334155}
.title .small{font-size:12px;margin-top:8px;color:#475569}
.datebox{background:#08244a;color:#fff;border-radius:0 0 0 18px;padding:18px 22px;font-size:12px;line-height:1.7;min-width:150px}
.main{display:grid;grid-template-columns:1.1fr 1fr .85fr;gap:18px;margin-top:16px}
.car-photo{height:205px;border-radius:14px;background:linear-gradient(135deg,#0f172a,#1e3a8a);display:flex;flex-direction:column;align-items:center;justify-content:center;border:1px solid #e5e7eb;color:white;text-align:center}.brand-big{font-size:38px;font-weight:900;letter-spacing:2px}.brand-model{font-size:22px;font-weight:800;margin-top:8px}.brand-line{font-size:12px;opacity:.8;margin-top:10px}
.car-name{font-size:24px;font-weight:900;color:#08244a;margin-bottom:4px}
.muted{color:#64748b;font-size:12px;line-height:1.55}
.health{border:1px solid #d1d5db;border-radius:14px;padding:16px;text-align:center}
.score{width:105px;height:105px;border-radius:60px;background:${statusColor};color:#fff;margin:12px auto;display:flex;align-items:center;justify-content:center;font-size:38px;font-weight:900}
.health-label{font-size:18px;font-weight:900;color:${statusColor}}
.kpis{display:grid;grid-template-columns:repeat(5,1fr);gap:12px;margin-top:16px}
.kpi{border:1px solid #d1d5db;border-radius:12px;padding:12px;text-align:center;min-height:86px}
.kpi .icon{font-size:22px;margin-bottom:5px}
.kpi .value{font-size:17px;font-weight:900;color:#08244a}
.kpi .label{font-size:11px;color:#334155;line-height:1.35}
.grid2{display:grid;grid-template-columns:1fr 1fr 1fr;gap:14px;margin-top:16px}
.box{border:1px solid #d1d5db;border-radius:14px;padding:14px;page-break-inside:avoid}
.box h2{font-size:15px;margin:0 0 12px;color:#08244a}
ul{padding-left:0;margin:0;list-style:none}
li{font-size:11.5px;padding:8px 0;border-bottom:1px solid #edf2f7;color:#1f2937}
li:before{content:'✓';color:#16a34a;font-weight:900;margin-right:8px}
table{width:100%;border-collapse:separate;border-spacing:0;overflow:hidden;border-radius:10px;border:1px solid #e5e7eb}
th{background:#f1f5f9;font-size:10.5px;text-transform:uppercase;color:#334155;padding:8px;border-bottom:1px solid #e5e7eb}
td{font-size:11px;padding:7px;border-bottom:1px solid #edf2f7;color:#111827}
tr:last-child td{border-bottom:0}
.status{font-weight:900;color:#16a34a}
.finance{font-size:22px;font-weight:900;color:#08244a;text-align:right}
.note{background:#ecfdf5;border-radius:12px;padding:12px;font-size:12px;color:#166534;line-height:1.5;margin-top:10px}
.footer{position:absolute;left:20px;right:20px;bottom:14px;background:#08244a;color:white;border-radius:0 14px 14px 0;padding:14px 18px;font-size:11px;display:flex;justify-content:space-between}
.page-num{background:#173b68;color:#fff;border-radius:10px 0 0 0;padding:10px 18px;position:absolute;right:0;bottom:0;font-weight:800;font-size:12px}
.hist-grid{display:grid;grid-template-columns:1fr 1fr;gap:14px;margin-top:12px}
.hist-box{border:1px solid #d1d5db;border-radius:14px;padding:10px;page-break-inside:avoid}
.hist-title{text-align:center;font-size:15px;font-weight:900;color:#08244a;margin-bottom:8px;text-transform:uppercase}
.total td{background:#f1f5f9;font-weight:900;color:#08244a}
.advice{display:grid;grid-template-columns:1fr 1fr;gap:14px;margin-top:16px}
.advice .box{min-height:70px}
</style>
</head>
<body>
...
<div class="page">
  <div class="header">
    ...
    <div class="title">
      <h1>RAPPORT DE SUIVI VÉHICULE</h1>
      <div class="sub">RAPPORT PROFESSIONNEL DE SUIVI ET D’ENTRETIEN</div>
      <div class="small">Présentation détaillée du suivi du véhicule destinée à valoriser l’entretien et la transparence lors de la vente</div>
    </div>
    <div class="datebox">
      Date du rapport<br/><b>${todayFr}</b><br/><br/>
      Généré par<br/><b>AutoCarnet</b>
    </div>
  </div>
...
<div class="car-photo brand-visual">
  <div class="brand-big">${brand}</div>
  <div class="brand-model">${car.modele||''}</div>
  <div class="brand-line">${fmtImmat(car.immat)} · ${fmtKm(car.km)}</div>
</div>


    <div>
      <div class="car-name">${car.marque} ${car.modele}</div>
      <div class="car-name" style="font-size:18px">${car.annee||''}</div>
      <div class="muted">Immatriculation : <b>${fmtImmat(car.immat)}</b><br/>Carburant : <b>${car.carburant||'-'}</b><br/>Mise en circulation : <b>${fmtDate(car.dateMEC)}</b></div>
    </div>
    <div class="health">
      <div style="font-size:13px;font-weight:900;color:#08244a">ÉTAT DE SANTÉ GLOBAL</div>
      <div class="score">${score}</div>
      <div class="health-label">${label}</div>
      <div class="muted" style="margin-top:8px">Basé sur l’historique d’entretien et les échéances</div>
    </div>
  </div>

  <div class="kpis">
    <div class="kpi"><div class="icon">🔧</div><div class="value">${allOps.length}</div><div class="label">Interventions enregistrées</div></div>
    <div class="kpi"><div class="icon">📋</div><div class="value">${allOps.length}</div><div class="label">Historique documenté</div></div>
    <div class="kpi"><div class="icon">🗓️</div><div class="value" style="font-size:13px">${nextRevision}</div><div class="label">Prochaine révision</div></div>
    <div class="kpi"><div class="icon">🛡️</div><div class="value" style="font-size:13px">${fmtDate(car.assurance?.echeance)}</div><div class="label">Assurance</div></div>
    <div class="kpi"><div class="icon">🧾</div><div class="value" style="font-size:13px">${car.vignette?.payee?'Payée':'À vérifier'}</div><div class="label">Vignette</div></div>
  </div>

  <div class="grid2">
    <div class="box">
      <h2>POINTS CLÉS POUR L’ACHETEUR</h2>
      <ul>${positives.slice(0,7).map(x=>`<li>${x}</li>`).join('')}</ul>
    </div>

    <div class="box">
      <h2>FICHE D’IDENTITÉ DU VÉHICULE</h2>
      <table>${vehicleRows.map(r=>`<tr><td>${r[0]}</td><td><b>${r[1]}</b></td></tr>`).join('')}</table>
    </div>

    <div>
      <div class="box">
        <h2>ÉCHÉANCES & SUIVI</h2>
        <table>${deadlineRows.map(r=>`<tr><td>${r[0]}</td><td><b>${r[1]}</b></td><td class="status">${r[2]==='ok'?'OK':r[2]==='warning'?'À suivre':r[2]==='urgent'?'Urgent':'N/A'}</td></tr>`).join('')}</table>
      </div>
  ...
  <div class="footer">
    <span>Rapport généré par AutoCarnet</span>
    <span>Document non contractuel — informations fournies par le propriétaire</span>
  </div>
...
<div class="page">
  <div class="hist-grid">
    ${tableCat('Révisions & vidanges',cat.revision)}
    ${tableCat('Pneus',cat.pneus)}
    ${tableCat('Freinage',cat.freins)}
    ${tableCat('Batterie',cat.batterie)}
    ${tableCat('Climatisation',cat.clim)}
    ${tableCat('Courroie / distribution',cat.distribution)}
    ${tableCat('Boîte / transmission',cat.boite)}
    ${tableCat('Liquides & niveaux',cat.liquides)}
    ${tableCat('Assurance / vignette / contrôle technique',cat.admin)}
    ${tableCat('Autres interventions',cat.autres)}
  </div>
  ...
  <div class="advice">
    <div class="box">
      <h2>NOTRE AVIS</h2>
      <div class="muted">
        Ce véhicule bénéficie d’un suivi ${score>=85?'rigoureux et complet':score>=70?'globalement rassurant':score>=55?'correct mais à compléter':'à contrôler'}.
        Les informations saisies permettent de présenter un historique clair et vérifiable.
      </div>
    </div>
    <div class="box">
      <h2>FACTURES & JUSTIFICATIFS</h2>
      <div class="muted">
        Le véhicule dispose d’un historique centralisé permettant une lecture claire du suivi et des interventions enregistrées.
      </div>
    </div>
  </div>
...
const shareSalesReportPdf=async(car,ops=[])=>{
  try{
    ...
    const html=buildSalesReportHtml(car,ops,photoDataUri);
    ...
    if(available){
      await Sharing.shareAsync(uri,{
        ...
        dialogTitle:'Partager le rapport de vente',
        UTI:'com.adobe.pdf'
      });
    }else{
...
const buildSalesReportText=(car,ops=[])=>{
  const score=healthScore(car,ops);
  const label=healthLabel(score);
  const lastRevision=[...ops].filter(o=>String(o.type||'').toLowerCase().includes('vidange')||String(o.type||'').toLowerCase().includes('révision')).sort((a,b)=>new Date(b.date)-new Date(a.date))[0];

  return `🚘 Rapport de suivi véhicule

${car.marque} ${car.modele} ${car.annee||''}
Immatriculation : ${fmtImmat(car.immat)}
Kilométrage : ${fmtKm(car.km)}

✅ Avis général : ${label}

📌 Éléments factuels :
• Dernière révision connue : ${lastRevision?`${fmtDate(lastRevision.date)} à ${fmtKm(lastRevision.km)}`:`${fmtDate(car.revision?.derniereDate)} à ${fmtKm(car.revision?.dernierKm)}`}
• Prochaine révision estimée : ${fmtKm(nextRevKm(car))}
• Assurance : ${assStatus(car)==='ok'?`à jour jusqu’au ${fmtDate(car.assurance?.echeance)}`:assStatus(car)==='warning'?`échéance proche ${fmtDate(car.assurance?.echeance)}`:'à vérifier'}
• Visite technique : ${carAge(car.dateMEC)<5?'non applicable actuellement':vtStatus(car)==='ok'?`à jour jusqu’au ${fmtDate(car.vt?.echeance)}`:'à vérifier'}
• Vignette : ${car.vignette?.payee?'payée / renseignée':'à vérifier'}
• Historique : ${ops.length} opération(s) enregistrée(s)

🧾 Derniers entretiens :
${recentOps(ops,5).length?recentOps(ops,5).map(o=>`• ${fmtDate(o.date)} - ${o.type} - ${fmtKm(o.km)}`).join('\n'):'• Aucun entretien détaillé saisi'}

📝 Synthèse :
Ce véhicule présente un suivi ${score>=85?'très rassurant':score>=70?'globalement rassurant':score>=55?'correct mais à compléter':'à contrôler'} selon les informations disponibles.
Les éléments principaux sont centralisés : kilométrage, révision, assurance, visite technique, vignette et historique d’entretien.`;
};
...
const SalesHealthReport=({car,ops})=>{
  ...
  const facts=[
    ...
    ['Visite technique',carAge(car.dateMEC)<5?'Non applicable actuellement':vtStatus(car)==='ok'?`À jour jusqu’au ${fmtDate(car.vt?.echeance)}`:'À vérifier'],
    ['Vignette',car.vignette?.payee?'Payée / renseignée':'À vérifier'],
    ['Historique saisi',`${ops.length} opération(s) enregistrée(s)`],
  ];

  const positives=[];
  if(revStatus(car)==='ok')positives.push('Révision suivie et prochaine échéance maîtrisée.');
  if(assStatus(car)==='ok')positives.push('Assurance à jour.');
  if(vtStatus(car)==='ok'||carAge(car.dateMEC)<5)positives.push('Situation visite technique conforme selon les informations saisies.');
  if(car.vignette?.payee)positives.push('Vignette renseignée comme payée.');
  if(ops.length>=3)positives.push('Historique d’entretien documenté avec plusieurs opérations.');
  if(positives.length===0)positives.push('Les informations principales du véhicule sont centralisées et vérifiables.');

  return(
    <>
      <Card style={{alignItems:'center',padding:24}}>
        <Text style={{fontSize:42}}>🚘</Text>
        <Text style={{fontSize:22,fontWeight:'900',color:C.text,marginTop:8,textAlign:'center'}}>{car.marque} {car.modele}</Text>
        <Text style={{fontSize:13,color:C.textLight,marginTop:4}}>{fmtImmat(car.immat)} · {car.annee||'Année non renseignée'}</Text>
        <View style={{marginTop:16,backgroundColor:score>=80?C.successLight:score>=60?C.warningLight:C.dangerLight,borderRadius:18,paddingHorizontal:18,paddingVertical:10}}>
          <Text style={{fontSize:15,fontWeight:'900',color:score>=80?C.success:score>=60?C.warning:C.danger}}>{label}</Text>
        </View>
        <Text style={{fontSize:12,color:C.textLight,textAlign:'center',lineHeight:18,marginTop:12}}>
          Avis basé sur les données saisies : kilométrage, révisions, échéances, historique et documents de suivi.
        </Text>
      </Card>

      <TouchableOpacity onPress={()=>shareSalesReportPdf(car,ops)} style={{backgroundColor:C.primary,borderRadius:14,paddingVertical:15,alignItems:'center',marginBottom:12}}>
        <Text style={{color:'#fff',fontSize:15,fontWeight:'800'}}>📄 Générer et partager le PDF</Text>
      </TouchableOpacity>

      <Card>
        <Text style={{fontSize:16,fontWeight:'900',color:C.text,marginBottom:10}}>✅ Points rassurants pour l’acheteur</Text>
        {positives.map((x,i)=><Text key={i} style={{fontSize:13,color:C.textMed,lineHeight:20,marginBottom:6}}>• {x}</Text>)}
      </Card>

      <Card>
        <Text style={{fontSize:16,fontWeight:'900',color:C.text,marginBottom:10}}>📌 Éléments factuels</Text>
        {facts.map((f,i)=><InfoRow key={i} label={f[0]} value={f[1]} last={i===facts.length-1}/>)}
      </Card>

      <Card>
        <Text style={{fontSize:16,fontWeight:'900',color:C.text,marginBottom:10}}>🧾 Derniers entretiens</Text>
        {recentOps(ops,5).length===0
          ? <Text style={{fontSize:13,color:C.textLight}}>Aucune opération d’entretien saisie.</Text>
          : recentOps(ops,5).map((o,i)=><InfoRow key={o.id||i} label={`${fmtDate(o.date)} · ${o.type}`} value={`${fmtKm(o.km)} · ${fmtMoney(o.montant)}`} />)}
      </Card>

      <Card>
        <Text style={{fontSize:16,fontWeight:'900',color:C.text,marginBottom:10}}>📝 Avis synthétique</Text>
        <Text style={{fontSize:13,color:C.textMed,lineHeight:21}}>
          Ce véhicule présente un suivi {score>=85?'très rassurant':score>=70?'globalement rassurant':score>=55?'correct mais à compléter':'à contrôler'} selon les informations disponibles.
          Les principaux éléments utiles pour un acheteur sont centralisés : kilométrage, historique d’entretien, révision, assurance, visite technique et vignette.
          Ce rapport permet de présenter le véhicule avec des éléments vérifiables plutôt qu’un simple discours commercial.
        </Text>
      </Card>
    </>
  );
      ...
      <ScrollView contentContainerStyle={{padding:16,paddingBottom:90}}>
        ...
        {!car?<Card style={{alignItems:'center',padding:35}}><Text style={{fontSize:42}}>📄</Text><Text style={{fontWeight:'800',fontSize:16,marginTop:8}}>Aucun rapport disponible</Text></Card>:(
          !report?<>
            ...
            <ReportTitleCard icon="🌍" title="Rapport global" subtitle="Vue consolidée de tous les véhicules et dépenses totales." onPress={()=>setReport('global')}/>
          </>:<>
            ...
            {report==='global'&&<Card><Text style={{fontSize:16,fontWeight:'900',marginBottom:10}}>🌍 Rapport global</Text><InfoRow label="Nombre véhicules" value={String(cars.length)}/><InfoRow label="Dépenses totales" value={fmtMoney(totalGlobal)} last/></Card>}
          </>
        )}
      </ScrollView>
    </SafeAreaView>
...
const MainTabs=()=>(
  <Tab.Navigator screenOptions={{headerShown:false,tabBarStyle:{backgroundColor:'#fff',borderTopWidth:1,borderTopColor:'#f3f4f6',paddingBottom:18,paddingTop:8,height:78},tabBarActiveTintColor:C.primary,tabBarInactiveTintColor:C.textLight,tabBarLabelStyle:{fontSize:11,fontWeight:'600'}}}>
