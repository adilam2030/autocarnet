...
const LoginScreen=({navigation})=>{
  ...
  return(
    <SafeAreaView style={{flex:1,backgroundColor:C.primary,paddingTop:SAFE_TOP,paddingBottom:SAFE_BOTTOM,paddingTop:SAFE_TOP,paddingBottom:SAFE_BOTTOM}}>
    ...
    <SafeAreaView style={{flex:1,backgroundColor:C.bg,paddingTop:SAFE_TOP,paddingBottom:SAFE_BOTTOM,paddingTop:SAFE_TOP,paddingBottom:SAFE_BOTTOM}}>
      ...
      <View style={{backgroundColor:C.primary,padding:20,paddingBottom:24}}>
        <View style={{flexDirection:'row',alignItems:'center',gap:12,marginBottom:16}}>
          <TouchableOpacity onPress={()=>report?setReport(null):navigation.goBack()} style={{backgroundColor:'rgba(255,255,255,0.15)',width:36,height:36,borderRadius:10,alignItems:'center',justifyContent:'center'}}>
...
const AddCarScreen=({route,navigation})=>{
  ...
  const[form,setForm]=useState({
    ...
    puissanceFiscale:String(ex?.puissanceFiscale||''),
  ...
  return(
    <SafeAreaView style={{flex:1,backgroundColor:'#fff',paddingTop:SAFE_TOP,paddingBottom:SAFE_BOTTOM,paddingTop:SAFE_TOP,paddingBottom:SAFE_BOTTOM}}>
      ...
      <KeyboardAwareScrollView
        contentContainerStyle={{padding:20,paddingBottom:480}}
...
const AddOpScreen=({route,navigation})=>{
  ...
  return(
    <SafeAreaView style={{flex:1,backgroundColor:'#fff',paddingTop:SAFE_TOP,paddingBottom:SAFE_BOTTOM,paddingTop:SAFE_TOP,paddingBottom:SAFE_BOTTOM}}>
      ...
      <ScrollView contentContainerStyle={{padding:20,paddingBottom:320}} keyboardShouldPersistTaps="handled">
...
const AssistantScreen=({navigation})=>{
  ...
  return(
    <SafeAreaView style={{flex:1,backgroundColor:C.bg,paddingTop:SAFE_TOP,paddingBottom:SAFE_BOTTOM,paddingTop:SAFE_TOP,paddingBottom:SAFE_BOTTOM}}>
      <StatusBar barStyle="dark-content" backgroundColor={C.bg}/>
      <View style={{backgroundColor:C.primary,padding:20,paddingBottom:24}}>
        <Text style={{color:'#fff',fontSize:24,fontWeight:'800'}}>Argus</Text>
...
const constructeurAdvice=(car,ops=[])=>{
  ...
  items.push({
    ...
    reco:'Tous les 10 000 km',
    status:km%10000<7000?'ok':'warn'
  });

  if(String(car?.transmission||'').toLowerCase().includes('auto')){
    items.push({
      title:'Vidange boîte automatique',
      reco:'Recommandée vers 60 000 à 80 000 km',
      status:km<60000?'ok':'warn'
    });
  }

  items.push({
    title:'Liquide refroidissement',
    reco:'Contrôle conseillé tous les 4 ans',
  ...
  if(km>80000){
    items.push({
      ...
      reco:'Vérification recommandée selon motorisation',
      status:'warn'
    });
  }
...
const buildSalesReportHtml=(car,ops=[])=>{
  ...
  const revs=allOps.filter(o=>normalize(o.type).includes('vidange')||normalize(o.type).includes('révision')||normalize(o.type).includes('revision'));
  const pneus=allOps.filter(o=>normalize(o.type).includes('pneu'));
  const freins=allOps.filter(o=>normalize(o.type).includes('frein'));
  const batterie=allOps.filter(o=>normalize(o.type).includes('batterie'));
  const autres=allOps.filter(o=>
    !revs.includes(o)&&
    !pneus.includes(o)&&
    !freins.includes(o)&&
    !batterie.includes(o)
  ...
  if(revs.length) positives.push('Historique des révisions documenté');
  if(pneus.length) positives.push('Interventions pneus enregistrées');
  if(freins.length) positives.push('Suivi du freinage renseigné');
...
.compact-header{display:flex;align-items:center;justify-content:space-between;gap:16px;border-bottom:1px solid #e5e7eb;padding-bottom:8px}.brand-line{white-space:nowrap;min-width:210px}.brand-name{font-size:22px;font-weight:900;color:#08244a}.brand-sep{color:#94a3b8;margin:0 7px}.brand-car{font-size:16px;font-weight:900;color:#b91c1c}.report-title{flex:1;text-align:right;font-size:22px;font-weight:900;color:#08244a}.report-title span{display:block;font-size:10px;font-weight:600;color:#64748b;margin-top:3px}
.header{
  display:flex;
...
<div class="page">
  ...
  <div class="header compact-header">
    <div class="brand-line">
      ...
      <span class="brand-car">${model} ${car.annee||''}</span>
    </div>

    <div class="report-title">
      Rapport de suivi véhicule
      <span>Généré le ${todayFr} par AutoCarnet</span>
    </div>
  </div>
  ...
  <div class="top">
    ...
    <div class="vehicle">
      ...
      <div class="meta">
        ...
        <div class="meta-card">
          <div class="meta-label">Immatriculation</div>
          <div class="meta-value">${fmtImmat(car.immat)}</div>
        </div>

        <div class="meta-card">
          <div class="meta-label">Kilométrage</div>
          <div class="meta-value">${fmtKm(car.km)}</div>
        </div>

        <div class="meta-card">
          <div class="meta-label">Carburant</div>
          <div class="meta-value">${car.carburant||'-'}</div>
        </div>

        <div class="meta-card">
          <div class="meta-label">Mise en circulation</div>
          <div class="meta-value">${fmtDate(car.dateMEC)}</div>
        </div>
    ...
    <div class="health">
      ...
      <div class="health-small">
        Basé sur l’historique d’entretien
        et les échéances renseignées
      </div>
  ...
  <div class="kpis">
    ...
    <div class="kpi">
      ...
      <div class="kpi-value">${allOps.length}</div>
      <div class="kpi-label">Interventions</div>
    </div>

    <div class="kpi">
      <div class="kpi-icon">📅</div>
      <div class="kpi-value">${fmtKm(car.revision?.dernierKm)}</div>
      <div class="kpi-label">Dernière révision</div>
    </div>

    <div class="kpi">
      <div class="kpi-icon">🛠️</div>
      <div class="kpi-value">${fmtKm(nextRevKm(car))}</div>
      <div class="kpi-label">Prochaine révision</div>
    </div>

    <div class="kpi">
      <div class="kpi-icon">🛡️</div>
      <div class="kpi-value">${fmtDate(car.assurance?.echeance)}</div>
      <div class="kpi-label">Assurance</div>
    </div>
  ...
  <div class="middle">
    ...
    <div class="block">
      <div class="block-title">Points rassurants pour l’acheteur</div>
    ...
    <div class="block">
      <div class="block-title">Échéances & suivi</div>

      <div class="point">
        ✓ Prochaine révision : <b>${fmtKm(nextRevKm(car))}</b>
      </div>

      <div class="point">
        ✓ Assurance : <b>${fmtDate(car.assurance?.echeance)}</b>
      </div>

      <div class="point">
        ✓ Contrôle technique :
        <b>${carAge(car.dateMEC)<5?'Non applicable':fmtDate(car.vt?.echeance)}</b>
      </div>

      <div class="point">
        ✓ Vignette :
        <b>${car.vignette?.payee?'Payée':'À vérifier'}</b>
      </div>
  ...
  <div class="history-section">
    ...
    ${renderTable('Révisions & vidanges', revs)}
    ${renderTable('Pneus', pneus)}
    ${renderTable('Freinage', freins)}
    ${renderTable('Batterie', batterie)}
    ${renderTable('Autres interventions', autres)}

  </div>

  <div class="footer">
    <div>Rapport généré par AutoCarnet</div>
    <div>Document non contractuel</div>
  </div>
    ...
    <>
      ...
      <Card>
        ...
        {recentOps(ops,5).length===0
          ...
          : recentOps(ops,5).map((o,i)=><View key={o.id||i} style={{paddingVertical:10,borderBottomWidth:1,borderBottomColor:'#f3f4f6'}}>
          <Text style={{fontSize:13,color:C.textMed,fontWeight:'700'}}>{fmtDate(o.date)} · {o.type}</Text>
          <Text style={{fontSize:13,color:C.primary,fontWeight:'800',marginTop:4}}>{fmtKm(o.km)}{Number(o.montant)?` · ${fmtMoney(o.montant)}`:''}</Text>
        </View>)}
      </Card>
...
const MainTabs=()=>(
  <Tab.Navigator screenOptions={{headerShown:false,tabBarStyle:{paddingBottom:42,paddingTop:3,height:96,backgroundColor:'#fff',borderTopWidth:1,borderTopColor:'#f3f4f6',paddingBottom:42,paddingTop:3,height:96},tabBarActiveTintColor:C.primary,tabBarInactiveTintColor:C.textLight,tabBarLabelStyle:{fontSize:11,fontWeight:'600'}}}>
    ...
    <Tab.Screen name="Argus" component={AssistantScreen} options={{tabBarLabel:'Assistant',tabBarIcon:({focused})=><Text style={{fontSize:focused?22:20,opacity:focused?1:0.6}}>🧠</Text>}}/>
