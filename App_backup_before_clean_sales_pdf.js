...
const buildSalesReportHtml=(car,ops=[])=>{
  ...
  const initials=brand.slice(0,3);
  ...
  const tableCat=(title,list)=>{
    ...
    return `
      <div class="hist-box">
        ...
        <table>
          <thead><tr><th>Date</th><th>Km</th><th>Opération</th><th>Garage</th><th>Montant</th></tr></thead>
          <tbody>
            ${list.map(o=>`
              <tr>
                ...
                <td>${o.type||'-'}</td>
                <td>${o.garage||'-'}</td>
                <td>${fmtMoney(o.montant)}</td>
              </tr>`).join('')}
            <tr class="total"><td>Total</td><td colspan="3">${list.length} intervention(s)</td><td>${fmtMoney(t)}</td></tr>
          </tbody>
        </table>
      </div>`;
  };
...
.logo{width:82px;height:82px;border-radius:50%;background:linear-gradient(135deg,#111827,#64748b);color:#fff;display:flex;align-items:center;justify-content:center;font-weight:900;font-size:24px;margin-right:22px}
.title{flex:1}
...
.car-photo{height:205px;border-radius:14px;background:linear-gradient(145deg,#f8fafc,#e2e8f0);display:flex;align-items:center;justify-content:center;font-size:74px;border:1px solid #e5e7eb}
...
<div class="page">
  <div class="header">
    ...
    <div class="title">
      ...
      <div class="sub">DOSSIER DE VENTE PROFESSIONNEL</div>
      <div class="small">Rapport factuel, clair et vérifiable pour rassurer l’acheteur</div>
    </div>
    <div class="datebox">
  ...
  <div class="kpis">
    ...
    <div class="kpi"><div class="icon">💰</div><div class="value">${fmtMoney(total)}</div><div class="label">Dépenses suivies</div></div>
  ...
  <div class="grid2">
    ...
    <div>
      ...
      <div class="box" style="margin-top:12px">
        ...
        <div class="muted">Total dépenses suivies</div>
        <div class="finance">${fmtMoney(total)}</div>
        <div class="muted">Nombre d’interventions : <b>${allOps.length}</b></div>
        <div class="note">Un suivi régulier permet de préserver la valeur du véhicule et de rassurer l’acheteur.</div>
      </div>
    </div>
  </div>
...
<div class="page">
  ...
  <div class="advice">
    ...
    <div class="box">
      ...
      <div class="muted">
        Les factures et justificatifs peuvent être demandés au propriétaire. Ils renforcent la confiance lors de la vente.
      </div>
    </div>
  </div>
