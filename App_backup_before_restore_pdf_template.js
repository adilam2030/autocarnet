  ...
  return(
    <SafeAreaView style={{flex:1,backgroundColor:C.bg,paddingTop:SAFE_TOP,paddingBottom:SAFE_BOTTOM}}>
      ...
      <ScrollView contentContainerStyle={{padding:16,paddingBottom:260}}>
        ...
        <TouchableOpacity onPress={()=>Alert.alert('Sauvegarde','Export local et sauvegarde cloud à développer.')}>
          <Card style={{flexDirection:'row',alignItems:'center',gap:14}}>
            <Text style={{fontSize:34}}>☁️</Text>
            <View style={{flex:1}}>
              <Text style={{fontSize:18,fontWeight:'900',color:C.text}}>Sauvegarde / Export</Text>
              <Text style={{fontSize:13,color:C.textLight,marginTop:3}}>
                Sauvegarde, restauration et export des données.
              </Text>
            </View>
            <Text style={{fontSize:26,color:C.primary}}>›</Text>
          </Card>
        ...
        <TouchableOpacity onPress={()=>Alert.alert('Paramètres','Compte, stockage et préférences seront remis ici proprement.')}>
          <Card style={{flexDirection:'row',alignItems:'center',gap:14}}>
            <Text style={{fontSize:34}}>⚙️</Text>
            <View style={{flex:1}}>
              <Text style={{fontSize:18,fontWeight:'900',color:C.text}}>Paramètres</Text>
              <Text style={{fontSize:13,color:C.textLight,marginTop:3}}>
                Compte, application, stockage et préférences.
              </Text>
            </View>
            <Text style={{fontSize:26,color:C.primary}}>›</Text>
          </Card>
...
const AssistantScreen=({navigation,compact=false})=>{
  ...
  const argus=car?estimateArgus(car,ops):{min:0,max:0,avg:0};
    ...
    <SafeAreaView style={{flex:1,backgroundColor:C.bg,paddingTop:SAFE_TOP,paddingBottom:SAFE_BOTTOM}}>
      ...
      <View style={{backgroundColor:C.primary,paddingHorizontal:20,paddingTop:20,paddingBottom:24}}>
        ...
        <Text style={{color:'rgba(255,255,255,0.75)',fontSize:13,marginTop:5}}>
          Estimation indicative, santé constructeur et conseils de vente
        </Text>
      </View>
      )}

      <ScrollView contentContainerStyle={{padding:16,paddingBottom:260}} keyboardShouldPersistTaps="handled">
        {cars.length>0&&(
          <Dropdown
            label="Véhicule"
        ...
        {!car?(
          ...
          <>
            <Card>
              <View style={{flexDirection:'row',justifyContent:'space-between',alignItems:'flex-start',gap:12}}>
                <View style={{flex:1}}>
                  ...
                  <Text style={{fontSize:13,color:C.textLight,marginTop:4}}>{car.annee||''} · {fmtKm(car.km)} · {car.carburant||''}</Text>
                </View>
              ...
              <View style={{marginTop:18,backgroundColor:'#eff6ff',borderRadius:18,padding:18}}>
                <Text style={{fontSize:12,fontWeight:'900',color:C.primary}}>ESTIMATION INDICATIVE</Text>
                <Text style={{fontSize:28,fontWeight:'900',color:'#08244a',marginTop:8}}>
                  {fmtMoney(argus.min)} - {fmtMoney(argus.max)}
                </Text>
                <Text style={{fontSize:12,color:C.textLight,marginTop:6}}>
                  Estimation non officielle basée sur les informations saisies dans AutoCarnet.
                </Text>
              </View>
            </Card>
            ...
            <Card>
              ...
              {reco.map((r,i)=>(
                <View key={i} style={{backgroundColor:'#f8fafc',borderWidth:1,borderColor:'#e5e7eb',borderRadius:14,padding:14,marginBottom:i===reco.length-1?0:10}}>
            ...
            <Card>
              ...
              <Text style={{fontSize:13,color:C.textMed,lineHeight:22}}>
                • Mettre en avant les entretiens récents{"\n"}
                • Mentionner les échéances à jour : assurance, vignette, VT{"\n"}
                • Préparer un rapport PDF de vente depuis le menu Plus{"\n"}
                • Garder un prix cible et un prix minimum de négociation{"\n"}
                • Présenter un véhicule propre avec photos claires
              </Text>
            </Card>
          </>
        )}
...
const shareSalesReportPdf=async(car,ops=[])=>{
  try{
    ...
    const rows=ops.length?ops.map(o=>`
      <tr>
        <td>${safe(fmtDate(o.date))}</td>
        <td>${safe(fmtKm(o.km))}</td>
        <td>${safe(o.type)}</td>
        <td>${safe(o.garage||'-')}</td>
      </tr>
    `).join(''):`<tr><td colspan="4">Aucune opération enregistrée</td></tr>`;
    ...
    <head>
      ...
      <style>
        ...
        .header{display:flex;justify-content:space-between;align-items:flex-start;border-bottom:2px solid #e5e7eb;padding-bottom:18px;margin-bottom:22px;}
        .brand{font-size:26px;font-weight:800;color:#0b3b75;}
        .sub{font-size:12px;color:#64748b;margin-top:4px;}
        .box{border:1px solid #e5e7eb;border-radius:14px;padding:16px;margin-bottom:16px;}
        .title{font-size:20px;font-weight:800;margin-bottom:12px;color:#0b3b75;}
        ...
        th{background:#f1f5f9;text-align:left;padding:9px;border:1px solid #e5e7eb;}
    ...
    <body>
      <div class="header">
        <div>
          <div class="brand">AutoCarnet</div>
          <div class="sub">Rapport professionnel de suivi véhicule</div>
        </div>
        <div class="sub">Date : ${safe(fmtDate(today()))}</div>
      </div>

      <div class="box">
        <div class="title">${safe(car.marque)} ${safe(car.modele)} ${safe(car.annee||'')}</div>
        <div class="grid">
          <div><div class="label">Immatriculation</div><div class="value">${safe(fmtImmat(car.immat))}</div></div>
          <div><div class="label">Kilométrage</div><div class="value">${safe(fmtKm(car.km))}</div></div>
          <div><div class="label">Carburant</div><div class="value">${safe(car.carburant||'-')}</div></div>
          <div><div class="label">Mise en circulation</div><div class="value">${safe(fmtDate(car.dateMEC))}</div></div>
        </div>
      </div>

      <div class="box">
        <div class="title">État de suivi</div>
        <div class="grid">
          <div><div class="label">Dernière révision</div><div class="value">${safe(fmtDate(car.revision?.derniereDate))} à ${safe(fmtKm(car.revision?.dernierKm))}</div></div>
          <div><div class="label">Prochaine révision</div><div class="value">${safe(fmtKm(nextRevKm(car)))}</div></div>
          <div><div class="label">Assurance</div><div class="value">${safe(fmtDate(car.assurance?.echeance))}</div></div>
          <div><div class="label">Vignette</div><div class="value">${car.vignette?.payee?'Payée':'À vérifier'}</div></div>
        </div>
      </div>

      <div class="box">
        <div class="title">Points rassurants pour l’acheteur</div>
        <p class="ok">✓ Historique centralisé dans AutoCarnet</p>
        <p class="ok">✓ Révision et échéances principales renseignées</p>
        <p class="ok">✓ Informations véhicule présentées clairement</p>
      </div>

      <div class="box">
        <div class="title">Historique des interventions</div>
        <table>
          <thead>
      ...
      <div class="footer">
        Document non contractuel — informations fournies par le propriétaire via AutoCarnet.
      </div>
    </body>
    </html>`;
