...
const buildSalesReportHtml=(car,ops=[])=>{
  ...
  const model=`${car.modele||''} ${car.annee||''}`.trim();
  ...
  const byCat={
    ...
    administratif:allOps.filter(o=>normalize(o.type).includes('assurance')||normalize(o.type).includes('vignette')||normalize(o.type).includes('visite technique')),
  };

  const categorizedIds=new Set(Object.values(byCat).flat().map(o=>o.id));
  byCat.autres=allOps.filter(o=>!categorizedIds.has(o.id));

  const total=allOps.reduce((sum,o)=>sum+(Number(o.montant)||0),0);
  const lastRevision=byCat.revision[0];

  const statusColor=score>=85?'#10b981':score>=70?'#2563eb':score>=55?'#f59e0b':'#ef4444';
  const statusBg=score>=85?'#ecfdf5':score>=70?'#eff6ff':score>=55?'#fffbeb':'#fef2f2';
  ...
  if(allOps.length>=3)positives.push('Historique d’entretien documenté avec plusieurs opérations.');
  if(byCat.revision.length>0)positives.push(`${byCat.revision.length} opération(s) de vidange/révision enregistrée(s).`);
  if(byCat.pneus.length>0)positives.push(`${byCat.pneus.length} intervention(s) pneus enregistrée(s).`);
  if(byCat.freins.length>0)positives.push(`${byCat.freins.length} intervention(s) freinage enregistrée(s).`);
  ...
  const rows=[
    ...
    ['Carburant',car.carburant||'Non renseigné'],
    ['Kilométrage actuel',fmtKm(car.km)],
    ['Date mise en circulation',fmtDate(car.dateMEC)],
  ...
  const catTable=(title,list)=>{
    if(!list || list.length===0)return '';
    const totalCat=list.reduce((s,o)=>s+(Number(o.montant)||0),0);
    return `
      <div class="section">
        <div class="section-title">${title}</div>
        <div class="mini">Nombre : ${list.length} · Montant documenté : ${fmtMoney(totalCat)}</div>
        <table>
          <tr><th>Date</th><th>Opération</th><th>Km</th><th>Garage</th><th>Montant</th><th>Note</th></tr>
          ${list.map(o=>`
            <tr>
  ...
  <head>
    ...
    <style>
      ...
      .cover {
        ...
        padding: 34px;
        border-radius: 24px;
        background: linear-gradient(135deg,#0f172a 0%,#1a56db 62%,#2563eb 100%);
        color:white;
        position:relative;
        overflow:hidden;
      }
      ...
      .topbar { display:flex; justify-content:space-between; align-items:flex-start; position:relative; z-index:2; }
      .brand-logo {
        width:92px;
        height:92px;
        border-radius:24px;
      ...
      .doc-label { text-align:right; font-size:12px; opacity:.85; line-height:1.5; }
      .cover-main { margin-top:82px; position:relative; z-index:2; }
      .cover-title { font-size:34px; line-height:1.15; font-weight:900; margin:0; letter-spacing:-.5px; }
      .cover-subtitle { font-size:15px; opacity:.86; line-height:1.6; margin-top:14px; max-width:520px; }
      .vehicle-name { margin-top:36px; font-size:28px; font-weight:900; }
      .vehicle-meta { margin-top:8px; font-size:14px; opacity:.82; }
      .health {
        margin-top:28px;
      ...
      .kpi-grid { display:grid; grid-template-columns:1fr 1fr; gap:14px; margin-top:34px; max-width:620px; }
      .kpi {
      ...
      .kpi-label { font-size:11px; opacity:.75; }
      .kpi-value { font-size:20px; font-weight:900; margin-top:6px; }
      .cover-footer { position:absolute; bottom:28px; left:34px; right:34px; font-size:11px; opacity:.72; border-top:1px solid rgba(255,255,255,.2); padding-top:14px; }
      .page { padding: 28px 10px 0 10px; }
      .section { margin-top:18px; border:1px solid #e5e7eb; border-radius:18px; padding:18px; page-break-inside:avoid; }
      .section-title { font-size:18px; font-weight:900; margin-bottom:10px; color:#111827; }
      .mini { font-size:12px; color:#6b7280; margin-bottom:10px; }
      table { width:100%; border-collapse:collapse; }
      td, th { padding:8px; border-bottom:1px solid #e5e7eb; font-size:12px; text-align:left; vertical-align:top; }
      th { background:#f8fafc; color:#374151; font-size:11px; text-transform:uppercase; }
      .label { color:#6b7280; width:36%; }
      ul { padding-left:20px; margin:0; }
      li { margin-bottom:8px; font-size:13px; line-height:1.45; }
      .summary { font-size:14px; line-height:1.65; color:#374151; }
      .footer { margin-top:28px; font-size:11px; color:#6b7280; text-align:center; border-top:1px solid #e5e7eb; padding-top:12px; }
    </style>
  </head>
  <body>
    <div class="cover">
      <div class="topbar">
        <div class="brand-logo">${brand.slice(0,3)}</div>
        <div class="doc-label">
          DOSSIER DE VENTE<br/>
          Rapport de suivi véhicule<br/>
          Généré par AutoCarnet
        </div>
      </div>

      <div class="cover-main">
        <div class="cover-title">Rapport professionnel<br/>de suivi véhicule</div>
        <div class="cover-subtitle">
          Une fiche claire, factuelle et présentable pour rassurer l’acheteur avec les éléments de suivi, d’entretien et d’échéances du véhicule.
        </div>

        <div class="vehicle-name">${brand} ${model}</div>
        <div class="vehicle-meta">${fmtImmat(car.immat)} · ${fmtKm(car.km)} · ${car.carburant||'Carburant non renseigné'}</div>

        <div class="health">${label}</div>

        <div class="kpi-grid">
          <div class="kpi"><div class="kpi-label">Opérations suivies</div><div class="kpi-value">${allOps.length}</div></div>
          <div class="kpi"><div class="kpi-label">Dépenses documentées</div><div class="kpi-value">${fmtMoney(total)}</div></div>
          <div class="kpi"><div class="kpi-label">Révisions / vidanges</div><div class="kpi-value">${byCat.revision.length}</div></div>
          <div class="kpi"><div class="kpi-label">Pneus / freins</div><div class="kpi-value">${byCat.pneus.length + byCat.freins.length}</div></div>
        </div>
      </div>

      <div class="cover-footer">
        Rapport basé sur les informations saisies par le propriétaire. Les éléments présentés sont destinés à faciliter la transparence lors de la vente.
      </div>
    </div>

    <div class="page">
      <div class="section">
        <div class="section-title">✅ Points rassurants pour l’acheteur</div>
        <ul>${positives.map(x=>`<li>${x}</li>`).join('')}</ul>
      </div>

      <div class="section">
        <div class="section-title">📌 Fiche véhicule et éléments factuels</div>
        <table>
          ${rows.map(r=>`<tr><td class="label">${r[0]}</td><td><strong>${r[1]}</strong></td></tr>`).join('')}
        </table>
      </div>

      ${catTable('🔧 Vidanges et révisions',byCat.revision)}
      ${catTable('🛞 Pneus',byCat.pneus)}
      ${catTable('🛑 Freins',byCat.freins)}
      ${catTable('🔋 Batterie',byCat.batterie)}
      ${catTable('❄️ Climatisation',byCat.clim)}
      ${catTable('⚙️ Courroie / distribution',byCat.distribution)}
      ${catTable('🚘 Boîte / transmission',byCat.boite)}
      ${catTable('🧪 Liquides et niveaux',byCat.liquides)}
      ${catTable('📄 Assurance, vignette, visite technique',byCat.administratif)}
      ${catTable('🧰 Autres interventions',byCat.autres)}

      <div class="section">
        <div class="section-title">📝 Avis synthétique</div>
        <div class="summary">
          ...
          Le rapport regroupe les éléments utiles à la vente : kilométrage, révisions, interventions mécaniques, pneus, freins, batterie, assurance, visite technique, vignette et historique complet.
          L’objectif est de donner à l’acheteur une vision claire, factuelle et vérifiable de l’état de suivi du véhicule.
        </div>
      </div>

      <div class="footer">
        Rapport généré par AutoCarnet — informations déclarées par le propriétaire.
      </div>
    </div>
  </body>
  </html>`;
