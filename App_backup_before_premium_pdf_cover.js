...
const buildSalesReportHtml=(car,ops=[])=>{
  ...
  const rows=[
    ['Véhicule',`${car.marque} ${car.modele} ${car.annee||''}`],
    ...
    ['Total opérations enregistrées',`${allOps.length} opération(s)`],
    ['Total dépenses suivies',fmtMoney(total)],
  ];
  ...
  const catTable=(title,list)=>{
    ...
    return `
      <div class="section">
        <h2>${title}</h2>
        <div class="mini">Nombre : ${list.length} · Montant suivi : ${fmtMoney(totalCat)}</div>
        <table>
          ...
          ${
            ...
            `).join('')
            : `<tr><td colspan="6">Aucune opération enregistrée dans cette rubrique</td></tr>`
          }
        </table>
      </div>
    `;
  ...
  <head>
    <meta charset="utf-8" />
    <style>
      body { font-family: Arial, sans-serif; color:#111827; padding:28px; background:#ffffff; }
      .header { background:#1a56db; color:white; padding:24px; border-radius:18px; }
      .title { font-size:26px; font-weight:800; margin:0; }
      .subtitle { font-size:13px; opacity:.9; margin-top:6px; }
      .car { margin-top:22px; font-size:22px; font-weight:800; }
      .immat { color:#6b7280; font-size:13px; margin-top:4px; }
      .badge { display:inline-block; margin-top:16px; padding:10px 16px; border-radius:999px; color:white; background:${statusColor}; font-size:15px; font-weight:800; }
      .section { margin-top:22px; border:1px solid #e5e7eb; border-radius:16px; padding:18px; page-break-inside:avoid; }
      .section h2 { font-size:17px; margin:0 0 10px 0; }
      ...
      .kpi { border:1px solid #e5e7eb; border-radius:14px; padding:14px; }
  ...
  <body>
    <div class="header">
      <p class="title">Dossier de vente véhicule</p>
      <div class="subtitle">Rapport factuel de suivi, d’entretien et de transparence pour l’acheteur</div>
    </div>

    <div class="car">${car.marque} ${car.modele} ${car.annee||''}</div>
    <div class="immat">${fmtImmat(car.immat)} · ${fmtKm(car.km)}</div>
    ...
    <div class="grid">
      ...
      <div class="kpi"><div class="kpi-label">Interventions pneus / freins</div><div class="kpi-value">${byCat.pneus.length + byCat.freins.length}</div></div>
    </div>

    <div class="section">
      <h2>✅ Points rassurants pour l’acheteur</h2>
      <ul>${positives.map(x=>`<li>${x}</li>`).join('')}</ul>
    </div>

    <div class="section">
      <h2>📌 Fiche véhicule et éléments factuels</h2>
      <table>
    ...
    <div class="footer">
      Rapport généré par AutoCarnet — sur la base des informations saisies par le propriétaire.
    </div>
  </body>
  </html>`;
};
