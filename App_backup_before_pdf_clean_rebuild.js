...
const buildSalesReportHtml=(car,ops=[],photoDataUri='')=>{
  ...
  const vehiclePhoto = photoDataUri
    ? `<img class="vehicle-photo-img" src="${photoDataUri}" />`
    : `<div class="vehicle-photo-empty">
         <div class="vehicle-photo-brand">${brand}</div>
         <div class="vehicle-photo-model">${model}</div>
       </div>`;
      ...
      <div class="hist-box">
        ...
        <table>
          ...
          <tbody>
            ...
            `).join('')}
            <tr class="total">
              <td colspan="4">${list.length} intervention(s) enregistrée(s)</td>
            </tr>
          </tbody>
        </table>
      </div>
...
.page{min-height:1090px;padding:24px 28px;box-sizing:border-box;position:relative}
...
.kpi-value{font-size:15px;font-weight:900;color:#08244a}
.kpi-label{font-size:11px;color:#334155;line-height:1.35;margin-top:3px}
...
<div class="page">
  <div class="header">
    <div class="logo-box">
      <div class="logo-main">${brand}</div>
      <div class="logo-sub">${model}</div>
    </div>

    <div class="title">
      <h1>RAPPORT DE SUIVI VÉHICULE</h1>
      <div class="sub">RAPPORT PROFESSIONNEL DE SUIVI ET D’ENTRETIEN</div>
      <div class="desc">Présentation détaillée du suivi du véhicule destinée à valoriser l’entretien et la transparence lors de la vente.</div>
    </div>
  ...
  <div class="hero">
    <div class="identity">
      <h2>${model}</h2>
      <div class="year">${car.annee||''}</div>

      <div class="id-item">Immatriculation<br/><b>${fmtImmat(car.immat)}</b></div>
      <div class="id-item">Carburant<br/><b>${car.carburant||'-'}</b></div>
      <div class="id-item">Mise en circulation<br/><b>${fmtDate(car.dateMEC)}</b></div>
    </div>

    <div style="display:flex;gap:18px;align-items:stretch">

      <div style="flex:1;border:1px solid #dbe3ea;border-radius:16px;padding:18px;background:#f8fafc">
        <div style="font-size:13px;font-weight:900;color:#08244a;margin-bottom:12px">
          SYNTHÈSE DU VÉHICULE
        </div>

        <div style="font-size:13px;line-height:1.75;color:#334155">
          • Véhicule suivi via AutoCarnet<br/>
          • Historique d’entretien centralisé<br/>
          • ${allOps.length} intervention(s) enregistrée(s)<br/>
          • Dernier kilométrage connu : <b>${fmtKm(car.km)}</b><br/>
          • Suivi des échéances administratives et mécaniques
        </div>
      </div>
      ...
      <div class="health">
        ...
        <div class="health-desc">Basé sur l’historique d’entretien et les échéances</div>
      </div>
  ...
  <div class="kpis">
    <div class="kpi"><div class="kpi-icon">🔧</div><div class="kpi-value">${allOps.length}</div><div class="kpi-label">Interventions enregistrées</div></div>
    ...
    <div class="kpi"><div class="kpi-icon">🗓️</div><div class="kpi-value">${nextRevision}</div><div class="kpi-label">Prochaine révision estimée</div></div>
  ...
  <div class="green-note">
    Le suivi régulier du véhicule renforce la lisibilité de son historique et la confiance de l’acheteur.
  </div>
...
<div class="page">
  ...
  <div class="hist-grid">
    ${historyHtml || '<div class="hist-box"><div class="hist-title">Aucun historique renseigné</div></div>'}
  </div>
