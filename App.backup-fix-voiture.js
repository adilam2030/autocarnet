...
const getRedirectUri = () =>
  'https://auth.expo.io/@adilam2031/autocarnet';

const saveGoogleToken = async (tokenResponse) => {
  const expiresAt = Date.now() + ((tokenResponse.expiresIn || 3500) * 1000);
  await AsyncStorage.setItem('g_access_token', tokenResponse.accessToken || '');
  await AsyncStorage.setItem('g_token_expires_at', String(expiresAt));

  if (tokenResponse.refreshToken) {
    await AsyncStorage.setItem('g_refresh_token', tokenResponse.refreshToken);
  }
...
const refreshGoogleToken = async () => {
  try {
    const refreshToken = await AsyncStorage.getItem('g_refresh_token');
    ...
    const r = await fetch(GOOGLE_DISCOVERY.tokenEndpoint, {
      method: 'POST',
    ...
    const expiresAt = Date.now() + ((d.expires_in || 3500) * 1000);
    await AsyncStorage.setItem('g_access_token', d.access_token);
    await AsyncStorage.setItem('g_token_expires_at', String(expiresAt));
...
const gSignIn = async () => {
  try {
    ...
    if (result.type !== 'success' || !result.params?.code) {
      return { success:false, error:'Connexion Google annulée ou refusée' };
    }
    ...
    const profile = await fetchGoogleUser(tokenResponse.accessToken);
...
const LoginScreen=({navigation})=>{
  ...
  const register=async()=>{
    ...
    await AsyncStorage.setItem('userName',cleanEmail.split('@')[0]);
    ...
    Alert.alert(
      ...
      [
        ...
        {text:'Réinitialiser',style:'destructive',onPress:async()=>{
          await AsyncStorage.multiRemove([LOCAL_EMAIL_KEY,LOCAL_PIN_KEY,'userName']);
          setEmail('');setPin('');setPinConfirm('');setError('');setMode('register');
        }}
      ]
    );
  };
      ...
      <KeyboardAwareScrollView contentContainerStyle={{flexGrow:1,justifyContent:'center',padding:24}} keyboardShouldPersistTaps="handled" enableOnAndroid>
        ...
        <View style={{backgroundColor:'#fff',borderRadius:24,padding:24}}>
          ...
          <Text style={{fontSize:13,color:C.textLight,marginBottom:22,lineHeight:20}}>
            {isRegister?'Saisissez votre adresse mail et créez un code PIN.':'Votre identifiant est mémorisé. Saisissez uniquement votre code PIN.'}
          </Text>
          ...
          <Input label="Adresse mail" value={email} editable={isRegister} autoCapitalize="none" keyboardType="email-address" onChangeText={v=>{setEmail(v);setError('');}} placeholder="exemple@mail.com"/>
